...
void map_apic_into_task(uint64_t task_cr3_phys) {
    map_pages((uint64_t *)(task_cr3_phys + kernel.hhdm),
              (uint64_t)kernel.ioapic_addr + kernel.hhdm,
              (uint64_t)kernel.ioapic_addr, 1,
              KERNEL_PFLAG_PRESENT | KERNEL_PFLAG_WRITE);
    map_pages((uint64_t *)(task_cr3_phys + kernel.hhdm),
              (uint64_t)kernel.lapic_addr,
              (uint64_t)kernel.lapic_addr - kernel.hhdm, 1,
              KERNEL_PFLAG_PRESENT | KERNEL_PFLAG_WRITE);
}
...
void init_apic(void) {
    ...
    if (verify_apic()) {
        ...
        printf("This device does not support APIC, but rather only legacy PIC. "
               "Halting.\n");
        HALT_DEVICE();
    }
    // disable pic
    ...
    map_pages((uint64_t *)(kernel.cr3 + kernel.hhdm),
              ...
              (uint64_t)madt->local_apic_addr, 1,
              KERNEL_PFLAG_PRESENT | KERNEL_PFLAG_WRITE);
    ...
    while (incremented < madt->header.length) {
        if (entry->entry_type == IOAPIC) {
            ...
            map_pages((uint64_t *)(kernel.cr3 + kernel.hhdm),
                      (uint64_t)this_ioapic->ioapic_addr + kernel.hhdm,
                      (uint64_t)this_ioapic->ioapic_addr, 1,
                      KERNEL_PFLAG_PRESENT | KERNEL_PFLAG_WRITE);
