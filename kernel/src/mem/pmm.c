...
void init_pmm(void) {
    ...
    for (size_t i = 0; i < kernel.memmap_entry_count; i++) {
        printf("|0x%p|0x%p|%s|\n", kernel.memmap[i].base,
               kernel.memmap[i].length, mem_types[kernel.memmap[i].type]);
        if (kernel.memmap[i].type != 0)
            continue; // must be marked usable
        list_len++;
        if (!kernel.pmm_chunklist) {
            kernel.pmm_chunklist = (struct list *)PAGE_ALIGN_UP(
                kernel.memmap[i].base + kernel.hhdm);
            list_init(kernel.pmm_chunklist);
        } else {
            list_insert(kernel.pmm_chunklist,
                        (struct list *)PAGE_ALIGN_UP(kernel.memmap[i].base +
                                                     kernel.hhdm));
        }
    ...
    printf("|==================|==================|======================|\n");
    if (!list_len) {
        printf("Failed to initialise physical memory allocator, cannot "
               "continue.\n");
        HALT_DEVICE();
    }
