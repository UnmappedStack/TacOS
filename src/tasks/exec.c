...
int execve(char *filename) {
    ...
    for (size_t i = 0; i < file_header.program_header_entry_count; i++) {
        ...
        if (program_header.type == 1) {
            ...
            map_pages((uint64_t*) current_task->pml4, program_header.virtual_address, header_data_phys, bytes_to_pages(program_header.size_in_memory), flags);
    ...
    alloc_pages((uint64_t*) current_task->pml4, USER_STACK_ADDR, USER_STACK_PAGES, KERNEL_PFLAG_WRITE | KERNEL_PFLAG_USER | KERNEL_PFLAG_PRESENT);
