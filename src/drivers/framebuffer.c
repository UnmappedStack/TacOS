...
Framebuffer boot_get_framebuffer() {
    ...
    return (Framebuffer){
        ...
        .bytes_per_pix =
            (*limine_framebuffer_request.response->framebuffers)->bpp / 8,
        .width_bytes =
            (*limine_framebuffer_request.response->framebuffers)->width *
            (*limine_framebuffer_request.response->framebuffers)->bpp / 8,
    };
}

void draw_pixel(uint64_t x, uint64_t y, uint32_t colour) {
    uint32_t *location = (uint32_t *)(((uint8_t *)kernel.framebuffer.addr) +
                                      y * kernel.framebuffer.pitch);
    location[x] = colour;
}
...
void fill_rect(uint64_t x, uint64_t y, uint64_t width, uint64_t height,
               uint32_t colour) {
    unsigned char *where =
        (unsigned char *)(((uint8_t *)kernel.framebuffer.addr) +
                          y * kernel.framebuffer.pitch) +
        x;
...
void scroll_pixels(size_t num_pix) {
    ...
    uintptr_t old_row_loc = (uintptr_t)kernel.framebuffer.addr +
                            (num_pix * kernel.framebuffer.pitch);
    for (size_t y = 0; y < max_height; y++) {
        memcpy((uint32_t *)new_row_loc, (uint32_t *)old_row_loc,
               kernel.framebuffer.width_bytes);
        new_row_loc += kernel.framebuffer.pitch;
        old_row_loc += kernel.framebuffer.pitch;
    }
    fill_rect(0, max_height, kernel.framebuffer.width, num_pix,
