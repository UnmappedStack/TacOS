...
void memregion_add_kernel(Memregion **list) {
    ...
    length_buffer = PAGE_ALIGN_UP((uint64_t)p_writeallowed_start -
                                  (uint64_t)p_kernel_start);
    add_memregion(list, (uint64_t)p_kernel_start, length_buffer / 4096, false,
                  KERNEL_PFLAG_PRESENT);
    length_buffer =
        PAGE_ALIGN_UP((uint64_t)p_kernel_end - (uint64_t)p_writeallowed_start);
    add_memregion(list, (uint64_t)p_writeallowed_start, length_buffer / 4096,
 ...
 * This is because you pass add_memregion a pointer to a Memregion*, and if it's
 * null, then it'll initialise a new list. Otherwise it just adds a new
 * memregion to the list as normal.
 */
...
/* Copies a Memregion from an old page tree to a new page tree and returns the
 * Memregion which can then be inserted into the list of the new task. If it's
 * mapped as read-only, then it should simply map it into the new memory space,
 * but otherwise it should copy the data as well.
 */
Memregion *memregion_clone(Memregion *original, uint64_t old_uint64_tree,
                           uint64_t new_uint64_tree) {
    printf("Copy memregion, vaddr = 0x%p, flags = 0b%b, len = %i\n",
           original->addr, original->flags, original->num_pages);
    if (original->flags & KERNEL_PFLAG_WRITE) {
        // copy data and map
        void *dest = (void *)(kmalloc(original->num_pages) + kernel.hhdm);
        size_t bytes_to_copy = original->num_pages * 4096;
        read_vmem((uint64_t *)old_uint64_tree, original->addr, dest,
                  bytes_to_copy);
        map_pages((uint64_t *)new_uint64_tree, original->addr,
                  (uint64_t)dest - kernel.hhdm, original->num_pages,
                  original->flags);
    } else {
        uint64_t phys =
            virt_to_phys((uint64_t *)old_uint64_tree, original->addr);
        map_pages((uint64_t *)new_uint64_tree, original->addr, phys,
                  original->num_pages, original->flags);
    }
    return original;
}
