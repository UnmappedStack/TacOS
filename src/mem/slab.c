...
Cache *init_slab_cache(size_t obj_size, const char *name) {
    ...
    if (strlen(name) > 19) {
        printf("[WARNING] Slab cache of name \"%s\" is longer than the maximum cache name of twenty characters. "
    ...
    Cache *new_cache = (Cache*) (kmalloc(bytes_to_pages(PAGE_ALIGN_UP(sizeof(Cache)))) + kernel.hhdm);
    *new_cache = (Cache) {
        .obj_size        = obj_size,
...
Slab *slab_find_addr(Cache *cache, void *ptr) {
    ...
    for (struct list *iter = cache->full_nodes.next; iter != &cache->full_nodes;) {
    ...
    for (struct list *iter = cache->partial_nodes.next; iter != &cache->partial_nodes;) {
        if (((Slab*) iter)->data < ptr &&
            ((Slab*) iter)->data_end > ptr) {
            return (Slab*) iter;
        }
    ...
    for (struct list *iter = cache->free_nodes.next; iter != &cache->free_nodes;) {
        if (((Slab*) iter)->data < ptr &&
            ((Slab*) iter)->data_end > ptr) {
            return (Slab*) iter;
        }
...
void cache_destroy(Cache *cache) {
    ...
    kfree((uintptr_t) cache - kernel.hhdm, bytes_to_pages(PAGE_ALIGN_UP(sizeof(Cache))));
}
