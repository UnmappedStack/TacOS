...
void map_sections(uint64_t pml4[], bool verbose) {
    ...
    for (size_t entry = 0; entry < kernel.memmap_entry_count; entry++) {
        ...
        if (entry_type == LIMINE_MEMMAP_USABLE ||
            ...
            if (verbose)
                printf("Trying to map entry #%i (%i pages, vaddr = %p, paddr = "
                       "%p)...",
                       entry, kernel.memmap[entry].length / 4096,
                       kernel.memmap[entry].base + kernel.hhdm,
                       kernel.memmap[entry].base);
            ...
            map_pages(pml4, kernel.memmap[entry].base + kernel.hhdm,
                      kernel.memmap[entry].base,
                      kernel.memmap[entry].length / 4096, flags);
            if (verbose)
                printf(" Success\n");
        }
    }
...
void map_kernel(uint64_t pml4[], bool verbose) {
    ...
    if (verbose)
        printf("Map read-only section of kernel from paddr 0x%p to vaddr %p, "
               "%i pages.\n",
    ...
    phys_buffer = kernel.kernel_phys_addr +
                  (writeallowed_start - kernel.kernel_virt_addr);
    if (verbose)
        printf("Map writable section of kernel from paddr 0x%p to vaddr %p, %i "
               "pages.\n",
